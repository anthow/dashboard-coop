
<article class="  grid grid-cols-5 grid-rows-2 gap-x-10 gap-y-10   w-11/12 m-auto 	">
	<section class="bg-third text-white p-4 col-span-2 col-row-2 rounded">
		<h2 class="text-2xl text-center font-black mb-5">Revenus individuels</h2>
		<ul class="text-lg">
			<li>Laura Laruelle:<br /> 0 €</li>
			<li>Julie Vanderbyse:<br /> 0 €</li>
			<li>Anthony Englebert:<br /> 0 €</li>
			<li>David Hainaut:<br /> 500 €</li>
			<li>Jeremy Scala: <br /> 0 €</li>
			<li>Thomas Dussart: <br /> 0 €</li>
			<li>Christophe Renard: <br /> 876,37 €</li>
		</ul>
	</section>

	<section class="bg-third text-white col-span-2 p-4 rounded">
		<h2 class="text-2xl text-center font-black mb-5">Revenus en attente</h2>
		<ul class="text-lg">
			<li>Laura Laruelle:<br /> 0 €</li>
			<li>Julie Vanderbyse:<br /> 0 €</li>
			<li>Anthony Englebert:<br /> 0 €</li>
			<li>David Hainaut:<br /> 1071,03 €</li>
			<li>Jeremy Scala: <br /> 0 €</li>
			<li>Thomas Dussart: <br /> 0 €</li>
			<li>Christophe Renard: <br /> 1539,63 €</li>
		</ul>
	</section>

	<section class="bg-third text-white h-min p-4 rounded">
		<h2 class="text-2xl text-center font-black mb-5">Etat du compte Smart</h2>
		<h3 class="text-2xl text-center font-black mb-10">1904,11  €</h3>
	</section>

	<section class="bg-third h-min text-white p-4 rounded">
		<h2 class="text-2xl text-center font-black mb-5">Chiffre d'affaire 2022</h2>
		<h3 class="text-2xl text-center font-black mb-5">1814 €</h3>
	</section>

	<section class="bg-primary 0 text-white p-4 col-span-6 rounded">
		<h2 class="text-2xl text-center font-black mb-5">activité du compte smart</h2>

		<table class=" table-auto m-auto">
			<thead class="bg-white space-y-4  text-primary">
				<tr>
					<th>Date</th>
					<th>Client</th>
					<th>Intitulé </th>
					<th>prestataires</th>
					<th>Bénéficiaire </th>
					<th>Type de transaction</th>
					<th>référence smart</th>
					<th>montant</th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<th>04/12/2021</th>
					<th>1250€</th>
					<th>refonte du site</th>
					<th>Anthony Englebert</th>
					<th />
					<th>Facture</th>
					<th>2021.PFV.096292</th>
					<th>1500</th>
				</tr>
			</tbody>
		</table>
	</section>
</article>
